				...
				//Singular title (no link applied)
					if (
						...
						if ( ( $helper = get_edit_post_link( get_the_ID() ) ) && is_page() ) {
							$args['title'] .= ' <a href="' . esc_url( $helper ) . '" class="entry-edit" title="' . esc_attr( sprintf( esc_html__( 'Edit the "%s"', 'receptar' ), the_title_attribute( array( 'echo' => false ) ) ) ) .'"></a>';
						}
			...
			function receptar_footer_top() {
				//Preparing output
					$output = "\r\n\r\n" . 
                                                ...
                                                . ' Left Turn Only || 2016 || Austin Texas';
